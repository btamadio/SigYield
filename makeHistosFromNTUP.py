...
for entry in range(nEntries):
    t.GetEntry(entry)
    avgBW+=t.weight_jet_SFFix70.at(0)
avgBW /= nEntries
...
for key in f.GetListOfKeys():
    if 'MetaData' in key.GetName():
        ...
        print 'DSID: %s nEntries = %i SumOfWeights = %i denom = %i' %(dsid,t.GetEntries(),f.Get(mdHistName).GetBinContent(3),denom)
if not mdHistName:
...
h_MJ3 = ROOT.TH1F('h_MJ3','M_{J}^{#Sigma}, n_{fatjet} #geq 3',150,0,1500)
h_MJ4 = ROOT.TH1F('h_MJ4','M_{J}^{#Sigma}, n_{fatjet} #geq 4',150,0,1500)
h_MJ = ROOT.TH1F('h_MJ','M_{J}^{#Sigma}',150,0,1500)
...
for entry in range(nEntries):
    ...
    w=rescale*1E6*lumi*t.weight/denom
    bw = t.weight_jet_SFFix70.at(0)*w/avgBW
...
#    bw=w
    ...
    if nFatJet == 3:
        if nBTag == 0:
            h_eventCat.Fill(1,bw)
        else:
            h_eventCat.Fill(2,bw)
        h_eventCat.Fill(3,bw)
        if mj > 200 and mj < 600:
            if nBTag == 0:
                h_eventCat.Fill(4,bw)
            else:
                h_eventCat.Fill(5,bw)
            h_eventCat.Fill(6,bw)
    if nFatJet == 4:
        if nBTag == 0:
            h_eventCat.Fill(7,bw)
        else:
            h_eventCat.Fill(8,bw)
        h_eventCat.Fill(9,bw)
        if mj > 200 and mj < 600:
            if nBTag == 0:
                h_eventCat.Fill(10,bw)
            else:
                h_eventCat.Fill(11,bw)
            h_eventCat.Fill(12,bw)
    if nFatJet >= 5:
        if nBTag == 0:
            h_eventCat.Fill(13,bw)
        else:
            h_eventCat.Fill(14,bw)
        h_eventCat.Fill(15,bw)
        if mj > 200 and mj < 600:
            if nBTag == 0:
                h_eventCat.Fill(16,bw)
            else:
                h_eventCat.Fill(17,bw)
            h_eventCat.Fill(18,bw)
    
    #Fill SR yields
    if nFatJet == 4:
        if nBTag >= 1:
            if mj > 600:
                h_sigYield.Fill(1,bw)
            if mj > 650:
                h_sigYield.Fill(2,bw)
            if mj > 700:
                h_sigYield.Fill(3,bw)
            if mj > 750:
                h_sigYield.Fill(4,bw)
            if mj > 800:
                h_sigYield.Fill(5,bw)
        if mj > 600:
            h_sigYield.Fill(6,bw)
        if mj > 650:
            h_sigYield.Fill(7,bw)
        if mj > 700:
            h_sigYield.Fill(8,bw)
        if mj > 750:
            h_sigYield.Fill(9,bw)
        if mj > 800:
            h_sigYield.Fill(10,bw)
    if nFatJet >= 5:
        if nBTag >= 1:
            if mj > 600:
                h_sigYield.Fill(11,bw)
            if mj > 650:
                h_sigYield.Fill(12,bw)
            if mj > 700:
                h_sigYield.Fill(13,bw)
            if mj > 750:
                h_sigYield.Fill(14,bw)
            if mj > 800:
                h_sigYield.Fill(15,bw)
        if mj > 600:
            h_sigYield.Fill(16,bw)
        if mj > 650:
            h_sigYield.Fill(17,bw)
        if mj > 700:
            h_sigYield.Fill(18,bw)
        if mj > 750:
            h_sigYield.Fill(19,bw)
        if mj > 800:
            h_sigYield.Fill(20,bw)
